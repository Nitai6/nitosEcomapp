...
description: Use when the user wants to create a YouTube thumbnail, design a video thumbnail, or generate thumbnail art for a YouTube video. Uses Gemini 3 Pro Image Preview (Nano Banana Pro) to generate professional, high-contrast thumbnails composited with the creator's headshot photos.
---

# YouTube Thumbnail Generator

Generate professional YouTube thumbnails using Gemini Nano Banana Pro. Produces 4 entirely different thumbnail variations at once, saves them individually, and creates a 2x2 comparison grid so the user can quickly pick the direction they like best.
...
1. **Color contrast** — Vivid/bright colors that pop against the background. Can also mean making your thumbnail contrast against competitors in your niche (study what the dominant visual style is in the niche — consider when a different approach might stand out in the feed).
2. **Large face with emotion** — A recognizable person OR an unknown face with a strong, clear emotion. At smaller channel sizes, recognition is low — so **emotion matters more than face**. The face emotion should match the feeling the viewer would have watching the video (shock, excitement, confidence).
...
6. **Aesthetic imagery** — Cinematic, symmetrical, soothing visuals. Not always the right fit but can work for certain topics.
...
### Step 1: Get the Topic & Gather Reference Images
...
**However, do ask about specific visual elements.** Before gathering reference images, ask the user if there are any specific logos, products, tools, screenshots, or other visual assets that should appear in the thumbnail. For example: "Should I include any specific logos (Claude, Cursor, etc.) or product shots?" This takes 5 seconds and avoids wasting a generation on the wrong references.

Pick the first available headshot from `.claude/skills/youtube-thumbnail/headshots/`. If the folder is empty, tell the user to add a headshot photo there first.

**Then, gather reference images.** Based on the topic, identify any visual assets that should appear in the thumbnail and fetch them before designing. These get passed to the generation script via `--reference` so Gemini can incorporate real logos, icons, and visuals instead of hallucinating them.
...
If the topic doesn't need any specific visual assets (rare), skip this step.
...
python3 .claude/skills/youtube-thumbnail/scripts/generate_thumbnail.py \
  --headshot ".claude/skills/youtube-thumbnail/headshots/{selected-headshot}" \
...
**IMPORTANT:** Always read `brand-style.md` (in this skill directory) before crafting prompts. It contains the creator's brand colors, typography, and visual identity rules that should inform every thumbnail.
...
- **Face emotion:** Must match the feeling the viewer would have watching the video. At smaller channel sizes, emotion > recognition. Always specify the emotion explicitly in prompts.
...
- **Competitor contrast awareness:** Study what the dominant visual style is in the creator's niche. Differentiate through bold text sizing, high saturation accent colors, and vivid contrast. Make sure thumbnails POP against the feed.
...
- [ ] **Stands out from competitors** — would this catch your eye against other content in the niche's feed?
