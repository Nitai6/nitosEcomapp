...
2. **Handle** -e.g., `@thepaulalex_`
3. **Verified** -show the blue checkmark? (default: true)
4. **Headshot** -path to profile photo in `assets/headshots/`. List available files with `ls assets/headshots/` and let the user pick, or use the first one found.
...
- **The title slide (slide 1) should ALWAYS have an image.** If the user doesn't provide one, search for a relevant visual with Tavily. A strong image on the first slide is critical for stopping the scroll.
...
```bash
python3 .claude/skills/instagram-thread-carousel/scripts/tavily-image-search.py "search query" workspace/carousel/<name>/ref --count 5
```
...
```bash
python3 .claude/skills/instagram-thread-carousel/scripts/website-screenshot.py "https://example.com" workspace/carousel/<name>/ref/homepage.png --width 1280 --height 800
```
...
Each carousel gets its own folder under `workspace/carousel/`. Pick a short, descriptive name based on the topic (e.g., `thumbnail-generator`, `claude-youtube`, `ai-agents-101`).

Folder structure:
```
workspace/carousel/<name>/
...
Create the config file at `workspace/carousel/<name>/config.json`:
...
```json
{
  "profile": {
    ...
    "handle": "@handle",
    "verified": true,
    "headshot": "assets/headshots/profile.jpg"
  },
  "slides": [
    {
      "tweets": [
      ...
      "tweets": [
        {
          ...
          "image": "workspace/carousel/<name>/ref/amex-card.jpg"
        }
      ]
    }
  ]
...
```bash
python3 .claude/skills/instagram-thread-carousel/scripts/thread-to-carousel.py workspace/carousel/<name>/config.json workspace/carousel/<name>
```
...
- **Clichés** -"Playing checkers while a small group is playing chess" means nothing. Everyone can say it.
...
If the user doesn't have enough screenshots or reference images to fill out a carousel, generate them with Gemini (Nano Banana Pro). The script produces **16:9 landscape images** designed to embed inside carousel slides below the tweet text. It has a built-in style guide that ensures dark, moody, cinematic results matching the carousel aesthetic.
...
python3 .claude/skills/instagram-thread-carousel/scripts/generate_carousel_image.py \
    ...
    --output workspace/carousel/{name}/ref/generated-image.png
```

**Options:**
- `--reference path/to/screenshot.png` -Pass existing images for context (e.g., a raw screenshot to stylize into a mockup)
- `--headshot path/to/headshot.png` -Include a person in the generated image (use `.claude/skills/instagram-thumbnail/headshots/tyler-headshot.png`)
- `--examples path/to/style-ref.png` -Style inspiration images

Then reference the generated image in the slide config:
```json
{"text": "slide text here", "image": "workspace/carousel/{name}/ref/generated-image.png"}
```
...
**Profile pic not found:** Check the path in the config. Headshots should be in `assets/headshots/`. The script draws a gray circle as a placeholder if no headshot is found.

**Animated slides look wrong or crash the session:** The script now outputs `.mp4` (via ffmpeg) for animated slides instead of GIF, preserving full color. A `.png` preview is also saved for chat display. If ffmpeg is not installed, it falls back to GIF (which has color limitations). Install ffmpeg with `brew install ffmpeg`.
