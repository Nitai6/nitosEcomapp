...
def measure_tweet(tweet, profile, fonts, max_text_w, draw):
    ...
    if tweet.get("image") and os.path.exists(tweet["image"]):
        h += TEXT_IMAGE_GAP
        if _is_video(tweet["image"]):
            vw, vh = _get_video_dimensions(tweet["image"])
            aspect = vh / vw
        else:
...
def _extract_video_frames(path, target_w, target_h, fps=30):
    """Extract frames from a video file using ffmpeg. Returns list of PIL Images."""
    ffmpeg_bin = shutil.which("ffmpeg") or "/opt/homebrew/bin/ffmpeg"
    tmpdir = tempfile.mkdtemp(prefix="carousel_frames_")
    frames = []
    try:
        subprocess.run(
            [ffmpeg_bin, "-y", "-i", path,
             "-vf", f"fps={fps},scale={target_w}:{target_h}:force_original_aspect_ratio=decrease,pad={target_w}:{target_h}:(ow-iw)/2:(oh-ih)/2",
             "-pix_fmt", "rgb24",
        ...
        print(f"  Warning: video frame extraction failed: {e}", file=sys.stderr)
...
def draw_tweet(canvas, draw, tweet, profile, fonts, x, y, max_text_w, skip_gif=False):
    ...
    if tweet.get("image") and os.path.exists(tweet["image"]):
        ...
        if skip_gif and _is_animated(tweet["image"]):
            # Don't draw - return placement info for animated compositing
            gif_rect = (x, ty, img_w, img_h, tweet["image"])
        elif is_vid:
            # Draw first frame of video as static preview
            frames, tmpdir = _extract_video_frames(tweet["image"], img_w, img_h, fps=1)
            if frames:
                resized = frames[0].resize((img_w, img_h), Image.LANCZOS)
...
def _slide_has_gif(tweets):
    """Check if any tweet in a slide references an animated GIF or video."""
    for t in tweets:
        img_path = t.get("image", "")
        if img_path and os.path.exists(img_path) and _is_animated(img_path):
            return True
    return False
...
def generate_gif_slide(slide_tweets, profile, fonts, output_path):
    ...
    # Step 2: Extract frames and composite each onto the base
    frames = []
    durations = []
    tmpdir = None

    if _is_video(gif_path):
        # Extract frames from video using ffmpeg
        video_fps = 30
        raw_frames, tmpdir = _extract_video_frames(gif_path, gw, gh, fps=video_fps)
        frame_duration_ms = int(1000 / video_fps)
        for frame in raw_frames:
            frame = round_corners(frame.resize((gw, gh), Image.LANCZOS), IMAGE_RADIUS)
    ...
    # Clean up temp directory from video frame extraction
    if tmpdir:
        shutil.rmtree(tmpdir, ignore_errors=True)
