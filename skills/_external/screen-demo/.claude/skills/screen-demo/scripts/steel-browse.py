...
(() => {
    ...
    // macOS-style arrow cursor built with createElement (avoids TrustedHTML CSP)
    ...
    path.setAttribute('d', 'M1 1L1 18.5L5.5 13.5L9.5 21L12 19.8L8 12L14.5 12L1 1Z');
    ...
    // Click ring - expanding circle effect on click
    const ring = document.createElement('div');
    ring.id = 'steel-click-ring';
    ring.style.cssText = [
        'position: fixed',
        'width: 44px',
        'height: 44px',
    ...
    // Follow mouse - cursor tip is at top-left of SVG
    document.addEventListener('mousemove', (e) => {
        cursor.style.left = e.clientX + 'px';
        cursor.style.top = e.clientY + 'px';
    }, { passive: true });

    // Click feedback - slight scale pulse + expanding ring
...
def _execute_single_action(page, action, action_type, wait_ms, elapsed_ms, moments):
    ...
    if action_type == "navigate":
        ...
        nth = action.get("nth", None)
        print(f"  [{elapsed_ms}ms] Click → {selector}{f' (nth={nth})' if nth is not None else ''}", file=sys.stderr)
        if nth is not None:
            locator = page.locator(selector).nth(nth)
            wait_state = "attached" if force else "visible"
            locator.wait_for(state=wait_state, timeout=10000)
        ...
        key = action["key"]
        print(f"  [{elapsed_ms}ms] Press → {key}", file=sys.stderr)
        ...
        print(f"  [{elapsed_ms}ms] Evaluate → {script[:80]}", file=sys.stderr)
        result = page.evaluate(script)
        moments.append({
            "time_ms": elapsed_ms,
            "type": "evaluate",
            "label": action.get("label", "Evaluate JS"),
        })
